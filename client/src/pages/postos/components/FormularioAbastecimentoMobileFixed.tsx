...
export const FormularioAbastecimentoMobileFixed: React.FC<FormularioAbastecimentoProps> = ({ 
  ...
  // Otimizado para evitar vazamento de memória - timeout único com cleanup
  useEffect(() => {
    if (!user?.name || !user?.id) return;
    
    const operatorName = user.name === "Administrador" ? "Operador" : user.name;
    
    // Único setTimeout com cleanup adequado
  ...
  // Função OTIMIZADA para carregar projetos - sem diagnósticos desnecessários
  // Corrige vazamento de memória em dispositivos Android com pouca RAM
  const loadProjectsMobile = useCallback(async () => {
    ...
    try {
      ...
      setLoadingError('Erro ao carregar projetos. Tente novamente.');
    } catch (error: any) {
      clearTimeout(timeoutId);
      
      if (error.name === 'AbortError') {
        setLoadingError('Conexão lenta. Verifique sua internet.');
      } else {
        setLoadingError('Erro de conexão. Tente novamente.');
      }
    }
