  ...
  const loadFuelCardHistory = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Filtrar apenas solicitações do GP02 (base system)
        const gp02Requests = allData.filter((item: any) => 
          item.origem_tipo === 'base_system' && 
          (item.base?.includes('GP02') || item.base?.includes('JACAREI'))
        );
        
        console.log('Solicitações GP02 filtradas:', gp02Requests);
        
        // Mapear dados da API para o formato esperado
        const mappedHistory = gp02Requests.map((item: any) => ({
          ...
          observacoes: `Base: ${item.base || 'GP02'}`,
        ...
        if (Array.isArray(projectsData)) {
          ...
          if (gp02Base) {
            ...
            const grupoPereiraProject = {
              id: gp02Base.project_id,
              name: gp02Base.project_name,
