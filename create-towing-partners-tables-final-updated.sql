-- Script final atualizado para criar estrutura completa de parceiros de guincho no Supabase
-- Inclui tabelas, views e funções necessárias para o funcionamento do módulo
-- Compatível com a estrutura existente no Supabase

-- 1. Tabela de Parceiros de Guincho (atualização para colunas obrigatórias)
CREATE TABLE IF NOT EXISTS towing_partners (
  ...
  cost_per_km NUMERIC(10,2) DEFAULT 7.50,
  ...
  pix_key VARCHAR(100),
  pix_type VARCHAR(20),
  service_types TEXT[] DEFAULT ARRAY['leve', 'médio', 'pesado'],
  payment_methods TEXT[] DEFAULT ARRAY['dinheiro', 'cartão', 'pix'],
...
INSERT INTO towing_partners (
  ...
  address, email, cnpj, city, region, 
  status, service_types, payment_methods
) 
VALUES 
  (5, 'Guincho Águia', 'Guincho Águia LTDA', 'José Silva', '11987654321', 
   'Av. Paulista, 1000, São Paulo, SP', 'contato@aguiaguincho.com.br', '12.345.678/0001-99', 'São Paulo', 'SP',
   'ativo', ARRAY['leve', 'médio', 'pesado']::text[], ARRAY['dinheiro', 'cartão', 'pix']::text[]),
  
  (6, 'Ford', 'Ford Serviços de Guincho Ltda', 'Atendimento Ford', '11912345678', 
   'Av. Industrial, 2500, São Bernardo do Campo, SP', 'guincho@ford.com.br', '98.765.432/0001-10', 'São Bernardo do Campo', 'SP',
   'ativo', ARRAY['leve', 'médio', 'pesado']::text[], ARRAY['dinheiro', 'cartão', 'pix']::text[]),
  
  (8, 'Caio Ramos de Souza', 'Ramos Guincho Express', 'Caio Ramos', '11998887766', 
   'Rua das Flores, 150, São Paulo, SP', 'caio@ramosguincho.com.br', '11.222.333/0001-44', 'São Paulo', 'SP',
   'ativo', ARRAY['leve', 'médio', 'pesado']::text[], ARRAY['dinheiro', 'cartão', 'pix']::text[]),
  
  (9, 'Claudio de Oliveira Silva', 'Oliveira Auto Socorro', 'Claudio Silva', '11977766655', 
   'Av. Interlagos, 1500, São Paulo, SP', 'claudio@oliveiraautosocorro.com.br', '33.444.555/0001-66', 'São Paulo', 'SP',
   'ativo', ARRAY['leve', 'médio', 'pesado']::text[], ARRAY['dinheiro', 'cartão', 'pix']::text[]),
  
  (10, 'Daiane do Vale Amaral', 'Vale Serviços de Guincho', 'Daiane Amaral', '11966655544', 
   'Rua Vergueiro, 2000, São Paulo, SP', 'daiane@valeguincho.com.br', '55.666.777/0001-88', 'São Paulo', 'SP',
   'ativo', ARRAY['leve', 'médio']::text[], ARRAY['dinheiro', 'pix']::text[]),
  
  (11, 'Delões Guinchos e Munck', 'Delões Guinchos e Munck LTDA', 'Roberto Delões', '11955544433', 
   'Av. das Nações Unidas, 12000, São Paulo, SP', 'contato@deloesguinchos.com.br', '77.888.999/0001-00', 'São Paulo', 'SP',
   'ativo', ARRAY['médio', 'pesado']::text[], ARRAY['dinheiro', 'cartão', 'pix']::text[]),
  
  (12, 'Fluxo Guinchos', 'Fluxo Guinchos e Serviços LTDA', 'Amanda Fluxo', '11944433322', 
   'Av. Morumbi, 5000, São Paulo, SP', 'contato@fluxoguinchos.com.br', '12.987.654/0001-32', 'São Paulo', 'SP',
   'ativo', ARRAY['leve', 'médio', 'pesado']::text[], ARRAY['dinheiro', 'cartão', 'pix']::text[]),
  
  (15, 'Allan de Souza Vieira', 'Vieira Serviços Automotivos', 'Allan Vieira', '11933322211', 
   'Rua Augusta, 500, São Paulo, SP', 'allan@vieiraservicos.com.br', '11.654.321/0001-23', 'São Paulo', 'SP',
   'ativo', ARRAY['leve', 'médio']::text[], ARRAY['dinheiro', 'cartão', 'pix']::text[])
