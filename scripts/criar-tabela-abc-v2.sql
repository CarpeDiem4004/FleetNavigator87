-- Script para criação da tabela e views para o Posto ABC V2
-- Baseado na mesma estrutura utilizada para Campinas V2 e Osasco V2
...
CREATE INDEX IF NOT EXISTS idx_abastecimentos_abc_v2_placa ON abastecimentos_posto_abc_v2(placa);
CREATE INDEX IF NOT EXISTS idx_abastecimentos_abc_v2_created_at ON abastecimentos_posto_abc_v2(created_at);
CREATE INDEX IF NOT EXISTS idx_abastecimentos_abc_v2_tipo_combustivel ON abastecimentos_posto_abc_v2(tipo_combustivel);
...
CREATE OR REPLACE FUNCTION atualizar_timestamp_tanque_abc_v2()
RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trigger_update_tanque_timestamp_abc_v2 ON configuracao_tanques_abc_v2;
CREATE TRIGGER trigger_update_tanque_timestamp_abc_v2
BEFORE UPDATE ON configuracao_tanques_abc_v2
FOR EACH ROW
EXECUTE FUNCTION atualizar_timestamp_tanque_abc_v2();

-- Inserção dos dados iniciais dos tanques se não existirem
INSERT INTO configuracao_tanques_abc_v2 (tanque, capacidade_maxima, nivel_atual, tipo_combustivel)
SELECT 'Tanque Principal', 18000.00, 9000.00, 'DIESEL'
WHERE NOT EXISTS (
    SELECT 1 FROM configuracao_tanques_abc_v2 WHERE tanque = 'Tanque Principal' AND tipo_combustivel = 'DIESEL'
);

INSERT INTO configuracao_tanques_abc_v2 (tanque, capacidade_maxima, nivel_atual, tipo_combustivel)
SELECT 'Tanque ARLA', 4000.00, 2000.00, 'ARLA'
WHERE NOT EXISTS (
    SELECT 1 FROM configuracao_tanques_abc_v2 WHERE tanque = 'Tanque ARLA' AND tipo_combustivel = 'ARLA'
);

-- Criar tabela de histórico para rastreabilidade
CREATE TABLE IF NOT EXISTS abastecimentos_posto_abc_v2_historico (
  id SERIAL PRIMARY KEY,
...
COMMENT ON TABLE abastecimentos_posto_abc_v2_historico IS 'Histórico de alterações para abastecimentos do Posto ABC V2';

-- Criar índice para consultas de histórico
CREATE INDEX IF NOT EXISTS idx_abastecimentos_abc_v2_historico_abastecimento ON abastecimentos_posto_abc_v2_historico(abastecimento_id);

-- Função para registrar alterações
CREATE OR REPLACE FUNCTION abastecimentos_posto_abc_v2_audit_trigger_func()
RETURNS TRIGGER AS $$
BEGIN
  IF (TG_OP = 'UPDATE') THEN
    INSERT INTO abastecimentos_posto_abc_v2_historico (abastecimento_id, acao, usuario, dados)
...
CREATE TRIGGER abastecimentos_posto_abc_v2_audit_trigger
AFTER UPDATE OR DELETE ON abastecimentos_posto_abc_v2
FOR EACH ROW EXECUTE FUNCTION abastecimentos_posto_abc_v2_audit_trigger_func();