-- Script corrigido para criar a tabela abastecimentos_posto_alair_v2
-- Observações:
...
-- Inserir configuração inicial para o posto Alair V2 se ainda não existir
INSERT INTO configuracao_tanques 
(posto, diesel_capacidade, diesel_nivel, diesel_valor_litro)
VALUES 
('Alair_v2', 18000, 9000, 5.79)
...
BEGIN
    ...
    ELSIF NEW.tipo_combustivel = 'Gasolina' THEN
        -- Se futuramente for implementado
        NULL;
    ...
    IF NEW.tipo_combustivel = 'Diesel' THEN
        SELECT diesel_nivel INTO NEW.nivel_tanque_apos FROM configuracao_tanques WHERE posto = 'Alair_v2';
    ELSIF NEW.tipo_combustivel = 'Arla' THEN
        SELECT arla_nivel INTO NEW.nivel_tanque_apos FROM configuracao_tanques WHERE posto = 'Alair_v2';
    END IF;
...
DROP TRIGGER IF EXISTS tr_atualizar_nivel_tanque_alair_v2 ON abastecimentos_posto_alair_v2;
CREATE TRIGGER tr_atualizar_nivel_tanque_alair_v2
...
BEGIN
    ...
    SELECT * INTO ultimo_abastecimento FROM abastecimentos_posto_alair_v2
    ...
    IF NEW.valor_unitario IS NULL OR NEW.valor_unitario = 0 THEN
        IF NEW.tipo_combustivel = 'Diesel' THEN
            SELECT diesel_valor_litro INTO NEW.valor_unitario FROM configuracao_tanques WHERE posto = 'Alair_v2';
        ELSIF NEW.tipo_combustivel = 'Arla' THEN
            SELECT arla_valor_litro INTO NEW.valor_unitario FROM configuracao_tanques WHERE posto = 'Alair_v2';
        END IF;
    END IF;
...
DROP TRIGGER IF EXISTS tr_calcular_consumo_alair_v2 ON abastecimentos_posto_alair_v2;
CREATE TRIGGER tr_calcular_consumo_alair_v2
BEFORE INSERT ON abastecimentos_posto_alair_v2
FOR EACH ROW
EXECUTE FUNCTION calcular_consumo_alair_v2();

-- Criar função para reabastecimento de tanque
CREATE OR REPLACE FUNCTION reabastecer_tanque_alair_v2(
...
CREATE OR REPLACE FUNCTION obter_valor_combustivel_alair_v2(p_tipo_combustivel TEXT) 
RETURNS NUMERIC AS $$
DECLARE
    valor NUMERIC;
BEGIN
    IF p_tipo_combustivel = 'Diesel' THEN
        SELECT diesel_valor_litro INTO valor FROM configuracao_tanques WHERE posto = 'Alair_v2';
    ELSIF p_tipo_combustivel = 'Arla' THEN
        SELECT arla_valor_litro INTO valor FROM configuracao_tanques WHERE posto = 'Alair_v2';
    ELSE
        valor := 0;
    END IF;
...
CREATE OR REPLACE VIEW view_alair_v2_estatisticas_por_base AS
SELECT 
    base,
...
CREATE INDEX IF NOT EXISTS idx_abastecimentos_alair_v2_placa ON abastecimentos_posto_alair_v2(placa);
CREATE INDEX IF NOT EXISTS idx_abastecimentos_alair_v2_data_hora ON abastecimentos_posto_alair_v2(data_hora);
CREATE INDEX IF NOT EXISTS idx_abastecimentos_alair_v2_tipo_combustivel ON abastecimentos_posto_alair_v2(tipo_combustivel);
CREATE INDEX IF NOT EXISTS idx_abastecimentos_alair_v2_base ON abastecimentos_posto_alair_v2(base);
CREATE INDEX IF NOT EXISTS idx_abastecimentos_alair_v2_status ON abastecimentos_posto_alair_v2(status);

-- Confirmar finalização
SELECT 'Tabela e configurações para posto Alair V2 criadas com sucesso.' as resultado;