...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/mobile/test-projects', async (req, res) => {
    try {
      console.log('[MOBILE-API] Requisição de teste de projetos para mobile recebida');
      console.log('[MOBILE-API] Headers:', req.headers);
      
      // Verificar se é uma requisição mobile
      const isMobile = req.headers['x-mobile-request'] === 'true' || 
                      /Mobile|Android|iPhone|iPad/i.test(req.headers['user-agent'] || '');
      
      // Buscar projetos com bases do Supabase
      const query = `
        SELECT 
          p.id,
          p.name,
      ...
      console.log(`[MOBILE-API] ${projects.length} projetos encontrados para mobile`);
      ...
      res.setHeader('X-Response-Source', 'mobile-api');
      ...
      return res.status(200).json({
        ...
        source: 'mobile-test-api'
      });
    } catch (error: any) {
      console.error('[MOBILE-API] Erro na API de teste mobile:', error);
      return res.status(500).json({
        success: false,
        message: 'Erro ao carregar projetos para mobile',
        error: error.message,
        source: 'mobile-test-api'
      });
    }
  ...
  app.get('/api/public/projects-with-bases', async (req, res) => {
    try {
      console.log('[PUBLIC-API] Requisição pública de projetos recebida');
      console.log('[PUBLIC-API] Headers:', req.headers);
      console.log('[PUBLIC-API] Origin:', req.headers.origin);
      
      // Headers CORS otimizados para links externos
      res.setHeader('Access-Control-Allow-Origin', '*');
      res.setHeader('Access-Control-Allow-Methods', 'GET, OPTIONS');
      res.setHeader('Access-Control-Allow-Headers', 'Content-Type, X-Mobile-Request, X-Strategy');
      res.setHeader('Cache-Control', 'no-cache');
      ...
      console.log(`[PUBLIC-API] ${projects.length} projetos retornados para API pública`);
      ...
      console.error('[PUBLIC-API] Erro na API pública:', error);
      return res.status(500).json({
        success: false,
