  ...
  try {
    ...
    if (postoName.toLowerCase() === 'campinas_v2' || 
      ...
      let columns = [
        ...
        tableColumns.includes('motorista') ? 'motorista as nome_motorista' : tableColumns.includes('nome_motorista') ? 'nome_motorista' : "'Não informado' as nome_motorista",
        tableColumns.includes('motorista_rg') ? 'motorista_rg as rg_motorista' : tableColumns.includes('rg_motorista') ? 'rg_motorista' : "NULL as rg_motorista",
        tableColumns.includes('operador') ? 'operador as nome_operador' : tableColumns.includes('nome_operador') ? 'nome_operador' : "'Sistema' as nome_operador",
  ...
  try {
    ...
    if (isV2Posto) {
      ...
      const fieldsToAdd = [
        ...
        { name: 'projeto_id', value: projeto_id },
        { name: 'base_id', value: base_id },
        { name: 'base_name', value: resolvedBaseName }
      ];
