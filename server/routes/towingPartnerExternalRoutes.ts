...
// Adaptação de tipos de request para evitar conflitos com tipos existentes
...
router.post('/generate', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (!req.isAuthenticated || !req.isAuthenticated() || !req.user || !['admin', 'gestor_frota'].includes(req.user.role)) {
 ...
 */
router.post('/revoke/:id', async (req: AuthenticatedRequest, res: Response) => {
  try {
    // Verificar autenticação do usuário
    if (!req.isAuthenticated || !req.isAuthenticated() || !req.user || !['admin', 'gestor_frota'].includes(req.user.role)) {
      return res.status(401).json({ error: 'Não autorizado' });
