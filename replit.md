...
### Outubro 2025
- **14/10/2025**: CORREÇÃO CRÍTICA de Timezone no Line Hall Fuel Card - bug onde data selecionada (dia 14) aparecia como dia anterior (dia 13) foi RESOLVIDO. **Root cause**: timezone UTC sendo convertido incorretamente. **Solução Backend**: Nova lógica em `lineHallRoutes.ts` extrai apenas a data (YYYY-MM-DD) sem timestamp, evitando conversão UTC. **Banco de Dados**: Adicionado campo `motivo_negacao` em `linehall_fuel_card_requests` + índices de performance (`idx_linehall_status`, `idx_linehall_data_solicitacao`). **Frontend**: Sistema de 4 abas (Pendentes|Atendidas|Negadas|Line Haul) agora exibe corretamente as datas com filtros funcionais.
