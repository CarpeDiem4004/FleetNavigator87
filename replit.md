...
### Outubro 2025
- **14/10/2025**: CORREÇÃO CRÍTICA de Timezone em TODOS os Sistemas de Fuel Card - bug onde data selecionada (dia 14) aparecia como dia anterior (dia 13) foi COMPLETAMENTE RESOLVIDO. **Root cause**: timezone UTC sendo convertido incorretamente em múltiplos endpoints. **Solução Backend**: Nova lógica aplicada em 2 arquivos (`lineHallRoutes.ts` linha 307-314 E `fuelCardSolicitationsApi.ts` linha 415-422) extrai apenas a data (YYYY-MM-DD) sem timestamp, evitando conversão UTC. **Banco de Dados**: Adicionado campo `motivo_negacao` em `linehall_fuel_card_requests` + índices de performance (`idx_linehall_status`, `idx_linehall_data_solicitacao`, `idx_solicitacoes_status`). **Frontend**: Sistema de 4 abas (Pendentes|Atendidas|Negadas|Line Haul) agora exibe corretamente as datas com filtros funcionais. **Arquivos Modificados**: `lineHallRoutes.ts`, `fuelCardSolicitationsApi.ts`, `FuelCardRequestsPanel.tsx`.
