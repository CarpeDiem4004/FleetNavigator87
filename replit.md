...
### Outubro 2025
- **11/10/2025**: Correção crítica no sistema de exportação Excel por período - bug de timezone resolvido. Queries de filtro por data agora usam `AT TIME ZONE 'America/Sao_Paulo'` antes do cast para date, garantindo que registros sejam filtrados pela data correta no horário do Brasil (não UTC). Anteriormente, registros criados às 23h do dia 10 no Brasil apareciam apenas ao selecionar dia 11. Correção aplicada em todas as três queries de exportação (`exportFuelCardSolicitationsByDate` em `fuelCardSolicitationsApi.ts`).
