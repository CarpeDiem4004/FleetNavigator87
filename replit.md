...
### Outubro 2025
- **18/10/2025**: Melhorias no painel de solicitações de fuel card - (1) Adicionado filtro por data de abastecimento (`data_uso`) com conversão correta de timezone usando `parseLocalDate()`, garantindo que registros sejam filtrados pela data correta no horário brasileiro **em todas as abas** (Pendentes, Atendidas, Negadas, Line Haul). (2) **TODAS as três abas principais** agora exibem breakdown de valores por provedor de cartão: **Pendentes** (verde), **Atendidas** (verde), **Negadas** (vermelho) - cada card mostra valor total + detalhamento separado por Ticket (💳 azul) e Alelo (💳 roxo), facilitando análise financeira completa. (3) **Modal de visualização de detalhes** agora exibe campo "Provedor de Cartão" com destaque visual: TICKET (fundo azul) ou ALELO (fundo roxo), facilitando identificação rápida do tipo de cartão. Implementação em `FuelCardRequestsPanel.tsx` com função `getValuesByCardType()` que filtra por campo `provedor_cartao` para cálculo correto dos valores. **Arquitetura**: Filtro global de data_uso aplica-se a todas as abas, exibição consistente com breakdown visual por tipo de cartão em todas as views, função otimizada com useCallback, cores semanticamente corretas (verde=positivo/aprovado, vermelho=negativo/rejeitado), modal com renderização condicional do campo provedor_cartao.
