...
### Outubro 2025
- **18/10/2025**: Melhorias no painel de solicitações de fuel card - (1) Adicionado filtro por data de abastecimento (`data_uso`) com conversão correta de timezone usando `parseLocalDate()`, garantindo que registros sejam filtrados pela data correta no horário brasileiro **em todas as abas** (Pendentes, Atendidas, Negadas, Line Haul). (2) **TODAS as três abas principais** agora exibem breakdown de valores por provedor de cartão: **Pendentes** (verde), **Atendidas** (verde), **Negadas** (vermelho) - cada card mostra valor total + detalhamento separado por Ticket (💳 VERDE) e Veloe Go (💳 ROXA), facilitando análise financeira completa. (3) **Modal de visualização de detalhes** agora exibe campo "Provedor de Cartão" com destaque visual: TICKET (fundo verde) ou VELOE GO (fundo roxo), facilitando identificação rápida do tipo de cartão. (4) **Migração completa de "Alelo" para "Veloe Go"** em todo o sistema - frontend (formulários, dashboards, painéis), backend (4.777 registros atualizados no banco), e links externos de todas as bases. Implementação em `FuelCardRequestsPanel.tsx` com função `getValuesByCardType()` que filtra por campo `provedor_cartao` para cálculo correto dos valores. **Arquitetura**: Filtro global de data_uso aplica-se a todas as abas, exibição consistente com breakdown visual por tipo de cartão em todas as views, função otimizada com useCallback, cores semanticamente corretas (verde=positivo/aprovado, vermelho=negativo/rejeitado), modal com renderização condicional do campo provedor_cartao. **Cores Oficiais**: TICKET = VERDE, VELOE GO = ROXA.
- **14/10/2025**: ✅ SOLUÇÃO DEFINITIVA DE TIMEZONE - REGRA DE OURO IMPLEMENTADA: "A data digitada no formulário DEVE aparecer EXATAMENTE igual no sistema, sem nenhuma conversão ou alteração". **Problema Original**: Data selecionada (14/10) aparecia como dia anterior (13/10) devido conversão UTC. **Solução Completa em 3 Camadas**: 
  ...
  3. **Frontend de Exibição** (`FuelCardRequestsPanel.tsx` linhas 28-37): Função `parseLocalDate()` interpreta datas como horário local brasileiro, evitando subtração de 3h do UTC
  **Arquivos Modificados**: `FuelCardRequestForm.tsx`, `solicitation.tsx`, `fuelCardSolicitationsApi.ts`, `FuelCardRequestsPanel.tsx`. **Resultado**: Sistema 100% confiável - data 14/10 selecionada = 14/10 exibida.
- **14/10/2025**: Adicionado campo `motivo_negacao` ao banco de dados - campo disponível em 3 tabelas (`solicitacoes_fuel_card.motivo_negacao`, `fuel_card_requests.rejection_reason`, `linehall_fuel_card_requests.observacoes_operador`) + índices de performance criados (`idx_linehall_status`, `idx_linehall_data_solicitacao`, `idx_solicitacoes_status`).
- **11/10/2025**: Cálculo Automático de Distância via Google Maps integrado ao Line Hall - ao cadastrar rotas, a distância é calculada automaticamente via Google Maps Distance Matrix API quando origem e destino são preenchidos. Backend endpoint seguro `/api/line-hall/calculate-distance` protege API key. Frontend com debounce de 1s, indicador visual de loading, mensagens de status coloridas (azul=consultando, verde=sucesso, vermelho=erro), campo readonly. Delay de 500ms no fechamento do modal garante visualização da mensagem de sucesso e atualização da lista.
- **11/10/2025**: Status "EM ANDAMENTO" adicionado ao Line Hall - nova opção de status disponível ao criar operações, com badge azul na visualização e mensagem informativa quando não há rotas cadastradas.
- **11/10/2025**: Campo "Motivo de Negação" implementado no sistema de fuel card - quando solicitação é negada, campo de texto obrigatório aparece para registrar o motivo. Backend valida obrigatoriedade e salva em todas as 3 tabelas (solicitacoes_fuel_card.motivo_negacao, fuel_card_requests.rejection_reason, linehall_fuel_card_requests.observacoes_operador). Frontend exibe motivo registrado em vermelho e valida antes de salvar.
