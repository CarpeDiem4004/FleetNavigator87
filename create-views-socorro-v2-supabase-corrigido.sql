-- Script para criar as views faltantes para o posto Socorro_v2 no Supabase
-- Versão corrigida para resolver o erro 'column r.operador does not exist'

-- Primeiro, vamos verificar a estrutura real da tabela recebimentos_posto_socorro_v2
...
--   -- operador pode ter um nome diferente, vamos usar um valor genérico
--   observacoes TEXT,
--   created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
--   updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
-- );

-- Criação da view historico_consolidado_socorro_v2 (CORRIGIDO)
CREATE OR REPLACE VIEW historico_consolidado_socorro_v2 AS
SELECT
...
-- Criação da view status_tanques_socorro_v2
CREATE OR REPLACE VIEW status_tanques_socorro_v2 AS
SELECT
  ct.id,
  ct.tipo_combustivel,
  ct.capacidade_total,
...
FROM configuracao_tanques_socorro_v2 ct;

-- Criação da view ultimos_abastecimentos_socorro_v2
CREATE OR REPLACE VIEW ultimos_abastecimentos_socorro_v2 AS
SELECT 
  a.id,
  a.placa,
  a.created_at,
...
-- Criação da view estatisticas_mensais_socorro_v2
CREATE OR REPLACE VIEW estatisticas_mensais_socorro_v2 AS
SELECT
...
-- Criação da view consumo_por_veiculo_socorro_v2
CREATE OR REPLACE VIEW consumo_por_veiculo_socorro_v2 AS
SELECT
  placa,
  tipo_combustivel,
  COUNT(*) AS total_abastecimentos,
...
-- Criação da view comparativo_combustiveis_socorro_v2
CREATE OR REPLACE VIEW comparativo_combustiveis_socorro_v2 AS
SELECT
  tipo_combustivel,
  COUNT(*) AS total_abastecimentos,
  SUM(litros) AS total_litros,
